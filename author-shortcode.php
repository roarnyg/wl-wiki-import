...
function generate_authorList( $atts ) {
	$result = "";
	if(isset($_GET["type"])){
		if($_GET["type"] == "municipality" || $_GET["type"] == "period" || $_GET["type"] == "gerne"){
			$result .= generate_authorList_customField($_GET["type"]);
		}elseif($_GET["type"] == "alphabetical"){
			$result .= generate_authorList_alphabetical();
		}elseif($_GET["type"] == "last-searched"){
			$result .= generate_authorList_latestSearch();
		}
	}
	return '<div class="author-list-page">'.$result.'</div>';
}
add_shortcode( 'authors', 'generate_authorList' );
...
function generate_authorList_customField($type){
	...
	switch ($type) {
		case 'municipality':
			$pageTitle .= __("municipality", 'avia_framework');
			$fieldId = FIELD_ID_MUNICIPALITY;	break;
		case 'period':
			$pageTitle .= __("period", 'avia_framework');
			$fieldId = FIELD_ID_PERIOD;	break;
		case 'gerne':
			$pageTitle .= __("gerne", 'avia_framework');
			$fieldId = FIELD_ID_GENRE;	break;
	}
	$result .='<h1 class="author-title-sort">'.$pageTitle.'</h1>';

	$azRange = array();

	$fieldObject = get_field_object($fieldId);
	if($fieldId==FIELD_ID_MUNICIPALITY){
	...
	foreach ($fieldObject["choices"] as $optionKey=>$optionValue) {
		...
		$azRange[] = $optionValue;
		$result .= "<ul>";
		$getPostsArgs	= array('post_type'=>array('post','author'),'category' => AUTHOR_CATEGORY_ID, 'posts_per_page' => -1);
		...
		if(count($getPostsArgs)>2){
			...
			foreach ( $posts as $post ) {
				$resultPost = array(
					...
					"first_name" => get_field(FIELD_NAME_FIRSTNAME,$post->ID),
					"last_name" => get_field(FIELD_NAME_LASTNAME,$post->ID)
					);
			   $postArray[] = $resultPost;
		  	}
		  	uasort($postArray, function($a, $b) {
			    return ($a["last_name"] == $b["last_name"]) ?  np_strcmp($a["first_name"], $b["first_name"]): np_strcmp($a["last_name"], $b["last_name"]);
			});
...
function generate_authorList_alphabetical(){
	...
	$getPostsArgs	= array(
                                                'post_type'=>array('post','author'),
                                                'fields'=>array('ids','post_title'),
						'category' => AUTHOR_CATEGORY_ID,
						'posts_per_page' => -1,
						'meta_key' => FIELD_NAME_LASTNAME,
						'orderby' => "meta_value",
						'order' => "ASC"
);
	$posts = get_posts( $getPostsArgs );
...
function generate_authorList_latestSearch(){
	$numberOfResult = 10;
	if(WEB_SITE=="NN"){
		$result = '<h1>'.__("Sist søkte").'</h1>';
	}else{
		$result = '<h1>'.__("Forfattere etter siste søkte").'</h1>';
	}
